...
def mine_block():
    ...
    blockchain.add_transaction(sender=node_addresss, receiver='Jessica',amount=1)
...
def is_valid():
    ...
    if is_valid:
        response = {'message': 'Bockchain válido.'}
    else:
...
def connect_node():
    ...
    if nodes is None: return "Sem nós.", 400
    for node in nodes: blockchain.add_node(node)
    response = {'message': 'Todos nós conectados. Blockchain contém os seguintes nós:',
